...
This library works wonderfully on its own, with no dependencies, but can get a speed boost by using 3rd party libraries to parse oEmbed data. To use a 3rd party Formatter, just be sure to require the library _before_ ruby-oembed.
...
**Note:** The built-in JSON Formatter (OkJson) has exhibited some problems in parsing certain UTF8 strings. If you're seeing unexpected OEmbed::ParseError errors in your application, try using another JSON Formatter.
...
Note: If during the `bundle install`, the installation of nokogiri is taking a long time (> a minute or two), try manually installing the gem using the following:
...
Thanks to {all who have made contributions}[https://github.com/judofyr/ruby-oembed/contributors] to this library, both large and small.
