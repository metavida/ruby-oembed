begin
  ...
  namespace :oembed do
    ...
    task :update_noembed do
      # Details at https://noembed.com/
      json_uri = URI.parse("http://noembed.com/providers")
      ...
      url_regexps = []
      services.each do |service|
        url_regexps += service['patterns'].map{|r| Regexp.new(r.strip) }
      end
      url_regexps.sort! { |a,b| a.to_s <=> b.to_s }
