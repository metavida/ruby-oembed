...
* Removed the YAML-powered JSON parsing backend for security reasons (Marcos Wright Kuhns)
* Added an OkJson-powered JSON parsing backend so that ruby-oembed can continue working without any dependencies on external gems (Marcos Wright Kuhns)
* Fix gem dependencies to support continued development/testing in Ruby 1.8
